...
const AnswerIntentHandler = {
  ...
  handle(handlerInput) {
    ...
    if (answerIsCorrect) {
    ...
    } else if (presidentId === -1) {
      // User wants to skip
      speakOutput = `${getCasualResponse()}. ${getCorrectionResponse()} ${getPresName(
        test.problems[test.questionNum].answer
      )}. `;
      ...
      if (test.attempts >= 2) {
        speakOutput = `${getWrongResponse()}. ${getCorrectionResponse()} ${getPresName(
          test.problems[test.questionNum].answer
        )}. `;
