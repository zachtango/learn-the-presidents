...
const StartedInProgressTestIntentHandler = {
  canHandle(handlerInput) {
    ...
    console.log(request);
    return (
      request.type === "IntentRequest" && request.intent.name === "TestIntent" && request.dialogState !== "COMPLETED"
    );
  },
  handle(handlerInput) {
    ...
    if (DIFFICULTY) {
      ...
      if (sessionAttributes.test) {
        ...
        // test just started
        return handlerInput.responseBuilder.speak(speakOutput).addElicitSlotDirective("difficulty").getResponse();
      }
    }
  },
};
